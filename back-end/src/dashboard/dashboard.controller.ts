  ...
  async getStats(@CurrentUser('id') userId: number, @CurrentUser('role') role: string) {
    ...
    const [
      ...
      this.prisma.scientificWork.findMany({
        where: baseWhere,
        select: {
          id: true, title: true, authors: true, status: true, type: true,
          level: true, createdAt: true, aiScore: true,
      ...
      this.prisma.scientificWork.count({
        where: { ...baseWhere, status: { in: ['SUBMITTED', 'REVIEW', 'OUTLINE_REVIEW', 'PROPOSAL_REVIEW'] } },
      }),
    ]);
